<template>
  ...
  <swiper :options="swiperOption" class="swiper-box" ref="mySwiper">
    <swiper-slide
       style="background-color: rgba(227,227,227,0.84)"
       :key="imgIndex"
       class="swiper-item"
       v-for="(src, imgIndex) in imgs">
      <img :data-src="src" class="swiper-lazy" @onload="gaga">
    ...
    data () {
      return {
        swiperOption: {
          ...
          freeModeMomentumRatio: 0.6, // 自由状态下，惯性
          spaceBetween: 10, // 两图之间的间距
//          slidesOffsetBefore: 15, // 第一张图和左边的偏移量
          slidesOffsetAfter: 15, // 最后一张图和右边的偏移量
          lazyLoadingInPrevNextAmount: Math.ceil((document.body.clientWidth - 15) / 140),
